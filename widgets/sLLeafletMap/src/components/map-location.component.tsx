    ...
    useEffect(() => {
        if (map) {
            ...
            if (position.mode === "marker") {
                setMapBounds(map, getMarkerBounds(markers));
            }

            if (position.mode === "geojson") {
                setMapBounds(map, getGeoJSONBounds(geoJSONData));
            }
        }
    }, [lastUpdateTimestamp, position.mode, position.latitude, position.longitude, markers.length, geoJSONData.length]);
